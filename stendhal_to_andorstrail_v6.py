#!/usr/bin/env python3
"""
stendhal_to_andorstrail_v6.py
=============================
Converts ALL Stendhal maps (every level, including all interior zones) into
Andor's Trail-compatible 32×32-tile TMX map chunks.

Changes from v5
---------------
* NO BLANK CHUNKS:
  After building each chunk's layer data, the script checks whether every
  visual layer (Ground, Object, Object2, Above, Above2) is entirely zero
  (no tiles placed at all).  Chunks that are entirely blank are skipped —
  no TMX file is written, and they are excluded from worldmap.xml,
  loadresources.xml, and the Tiled .world file.  A summary line is printed
  for each skipped chunk and a total count is shown in the final summary.

* CORRECT EDGE-CHUNK SIZING:
  Zones whose width or height is not an exact multiple of 32 previously
  produced right-edge and bottom-edge chunks that were padded to the full
  32×32 size with blank (GID=0) tiles.  v6 measures the actual tile count
  available for every chunk (stored as ``actual_w`` / ``actual_h`` in the
  chunk plan) and writes a correctly-sized TMX for those partial chunks
  instead of a padded 32×32 one.  This also prevents portal and exit
  coordinates from pointing into the blank padding area.

* WORLD FILE USES ACTUAL CHUNK SIZE:
  The Tiled .world file entry for each chunk now uses the correct pixel
  dimensions derived from the actual tile count, so Tiled's world view
  aligns edge chunks properly.
...
AT_VISUAL_LAYERS = {"Ground", "Object", "Object2", "Above", "Above2"}  # v6: used for blank detection
...
def is_blank_chunk(layer_tiles: dict, chunk_w: int, chunk_h: int) -> bool:
    ...
    a chunk that is all-blocked but has no visual tiles is still blank.
    ...
    for lname in AT_VISUAL_LAYERS:
        ...
        if tiles is None:
            continue
        # Only check the actual tile slots (not any oversized buffer)
        for i in range(n):
...
def probe_xml_structure(conf_dir: str, zone_by_name: dict):
    ...
    tags and attributes found inside <zone> elements.  This helps diagnose
    why portal parsing finds nothing.
    """
...
def generate_exits(map_id: str, walkable: list,
    ...
    actual tile boundary, not the padded 32×32 boundary.
    """
    W, H = chunk_w, chunk_h
...
def build_chunk_plan(zones_tmx: list):
    """
    v6 change: each chunk dict now contains ``actual_w`` and ``actual_h``
    reflecting the true number of tiles available (may be < AT_W / AT_H for
    right-edge and bottom-edge chunks).
    """
    ...
    for z, stmx in zones_tmx:
        ...
        for cy in range(ncy):
            for cx in range(ncx):
                ...
                # Skip degenerate chunks produced by rounding (zero-size)
                ...
                chunks.append({
                    ...
                    "world_ty": world_ty,
                    # v6: actual tile dimensions for this chunk
                    "actual_w": actual_w,
                    "actual_h": actual_h,
                })
...
def write_at_tmx(path: str,
                 ...
                 chunk_h: int = AT_H):
    """
    v6: chunk_w / chunk_h allow writing TMX files smaller than 32×32 for
...
def write_catalog(catalog_entries: list, exit_type_counts: dict):
    ...
    with open(cat_path, "w", encoding="utf-8") as fh:
        ...
        fh.write("Generated by stendhal_to_andorstrail_v6.py\n")
        fh.write("=" * 140 + "\n")
...
def main():
    print("=" * 70)
    print("  Stendhal → Andor's Trail map converter  v6")
    print("  (no blank tiles · correct edge-chunk sizing · bidirectional exits)")
    print("=" * 70)
    ...
    # ── 1. Zone configs ────────────────────────────────────────────
    print(f"\n[1/7] Loading zone configs from {STENDHAL_ZONE_CONF} ...")
    ...
    # ── 2. Load TMX files ──────────────────────────────────────────
    print(f"\n[2/7] Loading {len(zones)} TMX files ...")
    zones_tmx, skipped = [], 0
    for z in zones:
        ...
        try:
            ...
            # v6: warn if the source TMX has unexpected zero dimensions
            if stmx.width == 0 or stmx.height == 0:
                print(f"  [WARN] {fpath}: map has zero width or height "
                      f"({stmx.width}×{stmx.height}) — skipping")
                skipped += 1
                continue
    ...
    print(f"\n[3/7] Parsing ALL portal definitions ...")
    ...
    # ── 4. Chunk plan ──────────────────────────────────────────────
    print(f"\n[4/7] Building 32×32 chunk plan (v6: tracking actual edge sizes) ...")
    ...
    print(f"\n[5/7] Writing AT TMX map files (v6: skipping blank chunks) ...")
    output_records = []
    blank_skipped  = 0
    ...
    for cd in chunks:
        ...
        # v6: skip entirely blank chunks (no tiles in any visual layer)
        if is_blank_chunk(layer_tiles, actual_w, actual_h):
            blank_skipped += 1
            print(f"  [BLANK] skipped  {map_id}  ({actual_w}×{actual_h} tiles, all empty)")
            continue
        ...
        size_tag  = f"{actual_w}×{actual_h}" if (actual_w < AT_W or actual_h < AT_H) else "32×32"
        detail = f" [{size_tag}]"
    ...
    # v6: world file uses actual pixel dimensions per chunk (not always 32*TILE_PX)
    world_obj = {
        "type": "world",
        "maps": [
    ...
    print(f"  Edge chunks       : {edge_chunks}  (smaller than 32×32, sized to fit)")
